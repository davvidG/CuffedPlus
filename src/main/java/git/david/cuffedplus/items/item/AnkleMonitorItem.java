...
public class AnkleMonitorItem extends ArmorItem {
    ...
    public AnkleMonitorItem(ArmorMaterial armorMaterial, Type armorItem, Properties p) {
        super(armorMaterial, armorItem, p);
    }
/* THIS WILL BE AN ANKLE MONITOR THAT CAN BE PUT ON A PLAYER AND CONNECTED WITH THE TRACKER ITEM
    @Override
    public boolean onDroppedByPlayer(ItemStack item, Player player) {
        return false; // prevent from being dropped with Q
    }


    // Set an int value on the item
    public static void setAid(ItemStack stack, int value) {
        CompoundTag tag = stack.getOrCreateTag();
        tag.putInt("Aid", value);
    }

    // Get the int value from the item
    public static int getAid(ItemStack stack) {
        CompoundTag tag = stack.getTag();
        return tag != null ? tag.getInt("Aid") : 0;
    }
 */
}